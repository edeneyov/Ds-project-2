...
//					System.out.println("e2= " + e2);
				...
				if (e1 != e2) { 
					System.out.println("FAILED: delete, e1 != e2: " + e1 + ", " + e2); 
					return false; 
				}
	...
	public static boolean myTest(IHashTable ht) {
		
		for(int i =0; i<5;i++) {
			HashTableElement hte = new HashTableElement(i, i);
			try {
				ht.Insert(hte);
			}catch (Exception e) {
		...
		HashTableElement hte = new HashTableElement(1, 0);
		try {
			ht.Insert(hte);
		}catch (Exception e) {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 100; i++) { // later switch 6 to 100
